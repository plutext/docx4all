 ...
 * 
 * Intent is to be able to compare the state at a time we might save it to
 * the server (eg on leaving a content control), with some previously known
 * "clean" state.
 * 
 * Because pPr (eg style, alignment) can be changed from outside the content
 * control, it is not sufficient to just compare the value on exit to the
 * value on entry.
 * 
 */
public class ContentControlSnapshot {
	...
	public void refresh(SdtBlock newSdt) {
		// log.debug("refresh() invoked " + System.Environment.StackTrace);
		if (getId().equals(newSdt.getSdtPr().getId())) {
			this.cc = newSdt;
			...
			log.debug("Refreshed " + cc.getSdtPr().getId().getVal()
					+ " ContentControlSnapshot: " + pointInTimeXml);
		} else {
			throw new IllegalArgumentException(
				"New Sdt Id = " + newSdt.getSdtPr().getId().getVal()
				+ " while current Id = " + getId().getVal());
		}
	}
	...
	/*
	 * Strip the rsidRDefault attribute, which change between the time the
	 * document is opened and the time the content control is exited.
	 */
	...
	// node.Attributes.GetNamedItem("rsidRDefault",
	// Namespaces.WORDML_NAMESPACE)!=null) {
	...
	//
	// */
	// return doc.SelectSingleNode("//*[./w:sdtPr/w:id/@w:val='" + chunkID + "']",
	// nsmgr);
	// }
